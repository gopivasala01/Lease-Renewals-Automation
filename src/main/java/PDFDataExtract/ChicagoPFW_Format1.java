				   ...
				   System.out.println("Expiration Date = "+PDFReader.expirationDate);
				    ...
				    if(text.contains(PDFAppConfig.ChicagoPFW_Format1.residentBenefitsPackageCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
				    {
				    	...
				    	//HVAC Air Filter Fee
				    	 try
						    {
						    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.ChicagoPFW_Format1.RBP_Prior)+PDFAppConfig.ChicagoPFW_Format1.RBP_Prior.length()).trim().split(" ")[0];
