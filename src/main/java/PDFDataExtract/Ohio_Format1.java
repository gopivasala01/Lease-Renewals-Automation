			   ...
			   System.out.println("Expiration Date = "+PDFReader.expirationDate);
			    ...
			    if(text.contains(PDFAppConfig.Ohio_Format1.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
			    {
			    	...
			    	//HVAC Air Filter Fee
			    	 try
					    {
					    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Ohio_Format1.RBP_Prior)+PDFAppConfig.Ohio_Format1.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
