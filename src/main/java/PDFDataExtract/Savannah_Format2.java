...
public class Savannah_Format2 
{
	//public static void main(String[] args)
			public static boolean savannah() throws Exception
			{
			   ...
			   /* try
			    {
			    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.Savannah_Format2.monthlyRent_Prior)+PDFAppConfig.Savannah_Format2.monthlyRent_Prior.length()).trim().split(" ")[0];
			    ...
			    if(text.contains(PDFAppConfig.Savannah_Format2.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
			    {
			    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
			    	//HVAC Air Filter Fee
			    	 try
					    {
					    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Savannah_Format2.RBP_Prior)+PDFAppConfig.Savannah_Format2.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
