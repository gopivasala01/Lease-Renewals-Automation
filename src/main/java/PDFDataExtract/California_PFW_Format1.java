...
public class California_PFW_Format1 
{
	...
	public static boolean californiapfw() throws Exception
	{
	    ...
	    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
		    try
		    {
		    	PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.commencementDate_Prior)+PDFAppConfig.California_PFW_Format1.commencementDate_Prior.length(),text.indexOf(PDFAppConfig.California_PFW_Format1.commencementDate_After));
		    }
		    catch(Exception e)
		    {
		   ...
		   try
		    {
			   PDFReader.expirationDate = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.expirationDate_Prior)+PDFAppConfig.California_PFW_Format1.expirationDate_Prior.length(),text.indexOf(PDFAppConfig.California_PFW_Format1.expirationDate_After));
		   ...
		   /* try
		    {
		    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.monthlyRent_Prior)+PDFAppConfig.California_PFW_Format1.monthlyRent_Prior.length()).trim().split(" ")[0];
		    	if(PDFReader.monthlyRent.matches(".*[a-zA-Z]+.*"))
		    		PDFReader.monthlyRent = "Error";
		    ...
		    if(text.contains(PDFAppConfig.California_PFW_Format1.HVACFilterAddendumTextAvailabilityCheck))
		    {
		    	PDFReader.HVACFilterFlag = true;
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.HVACAirFilterFee = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.HVACAirFilterFee)+PDFAppConfig.California_PFW_Format1.HVACAirFilterFee.length()).trim().split(" ")[0];
		    ...
		    if(text.contains(PDFAppConfig.California_PFW_Format1.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
		    {
		    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.RBP_Prior)+PDFAppConfig.California_PFW_Format1.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
		    ...
		    try
		    {
		    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.prorateRent_Prior)+PDFAppConfig.California_PFW_Format1.prorateRent_Prior.length()).trim().split(" ")[0];
			...
			//Pet Rent
		    if(text.contains(PDFAppConfig.California_PFW_Format1.petAgreementAvailabilityCheck))
		    {
		    	...
		    	try
		    	{
		    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.petRent_Prior)+PDFAppConfig.California_PFW_Format1.petRent_Prior.length()).trim().split(" ")[0].trim();
		    ...
		    try
	    	{
	    		PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.California_PFW_Format1.leaseRenewalFee_Prior)+PDFAppConfig.California_PFW_Format1.leaseRenewalFee_Prior.length()).trim().split(" ")[0].trim();
