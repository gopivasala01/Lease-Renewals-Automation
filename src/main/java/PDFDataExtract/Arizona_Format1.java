	...
	//public static void main(String[] args) 
		public static boolean arizona() throws Exception
		{
		    ...
		    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
			    try
			    {
			    	PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.commencementDate_Prior)+PDFAppConfig.Arizona_Format1.commencementDate_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.commencementDate_After));
			    }
			    catch(Exception e)
			    {
			   ...
			   try
			    {
				   PDFReader.expirationDate = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.expirationDate_Prior)+PDFAppConfig.Arizona_Format1.expirationDate_Prior.length(),text.indexOf(PDFAppConfig.Arizona_Format1.expirationDate_After));
			   ...
			   System.out.println("Expiration Date = "+PDFReader.expirationDate);
			    ...
			    if(text.contains(PDFAppConfig.Arizona_Format1.residentBenefitsPackageCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
			    {
			    	...
			    	//HVAC Air Filter Fee
			    	 try
					    {
					    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Arizona_Format1.RBP_Prior)+PDFAppConfig.Arizona_Format1.RBP_Prior.length()).trim().split(" ")[0];
