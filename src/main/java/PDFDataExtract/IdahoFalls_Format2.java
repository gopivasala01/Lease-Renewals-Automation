	...
	public static boolean idahoFalls() throws Exception
	{
	    ...
	    if(text.contains(PDFAppConfig.IdahoFalls_Format2.residentBenefitsPackageCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
	    {
	    	...
	    	//HVAC Air Filter Fee
	    	 try
			    {
			    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.IdahoFalls_Format2.RBP_Prior)+PDFAppConfig.IdahoFalls_Format2.RBP_Prior.length()).replace("/", "").trim().split(" ")[0];
