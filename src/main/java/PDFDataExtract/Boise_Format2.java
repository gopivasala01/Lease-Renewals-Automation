	...
	public static boolean boise() throws Exception
	{
	    ...
	    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
		   ...
		   System.out.println("Expiration Date = "+PDFReader.expirationDate);
		    ...
		    if(text.contains(PDFAppConfig.Boise_Format1.residentBenefitsPackageCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
		    {
		    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Boise_Format1.RBP_Prior)+PDFAppConfig.Boise_Format1.RBP_Prior.length()).replace("/", "").trim().split(" ")[0];
		    ...
		    try
		    {
		    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Boise_Format1.prorateRent_Prior)+PDFAppConfig.Boise_Format1.prorateRent_Prior.length()).trim().split(" ")[0];
			...
			//Pet Rent
		    if(text.contains(PDFAppConfig.Boise_Format1.petAgreementAvailabilityCheck))
		    {
