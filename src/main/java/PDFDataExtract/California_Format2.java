...
public class California_Format2 
{
	...
	public static boolean california() throws Exception
	{
	   ...
	   /* try
	    ...
	    if(text.contains(PDFAppConfig.California_Format2.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
	    {
	    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
	    	//HVAC Air Filter Fee
	    	 try
			    {
			    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.California_Format2.RBP_Prior)+PDFAppConfig.California_Format2.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
