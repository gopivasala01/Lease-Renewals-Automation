	...
	public static boolean montana() throws Exception
	{
	    ...
	    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
		   ...
		   try {
		    ...
		    if(text.contains(PDFAppConfig.Montana_Format1.HVACFilterAddendumTextAvailabilityCheck))
		    {
		    	PDFReader.HVACFilterFlag = true;
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.HVACAirFilterFee = text.substring(text.indexOf(PDFAppConfig.Montana_Format1.HVACAirFilterFee)+PDFAppConfig.Montana_Format1.HVACAirFilterFee.length()).trim().split(" ")[0];
		    ...
		    if(text.contains(PDFAppConfig.Montana_Format1.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM"))))
		    {
		    	...
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Montana_Format1.RBP_Prior)+PDFAppConfig.Montana_Format1.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
		    ...
		    if(text.contains(PDFAppConfig.Montana_Format1.petAgreementAvailabilityCheck))
		    {
		    	...
		    	try
		    	{
		    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Montana_Format1.petRent_Prior)+PDFAppConfig.Montana_Format1.petRent_Prior.length()).trim().split(" ")[0].trim();
		    ...
		    try
	    	{
	    		PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.Montana_Format1.leaseRenewalFee_Prior)+PDFAppConfig.Montana_Format1.leaseRenewalFee_Prior.length()).trim().split(" ")[0].trim();
