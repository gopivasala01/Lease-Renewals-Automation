			...
			public static boolean sanAntonio() throws Exception
			{
			    ...
			    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
				   ...
				   System.out.println("Expiration Date = "+PDFReader.expirationDate);
				    ...
				    try
				    {
				    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.monthlyRent_Prior)+PDFAppConfig.SanAntonio_Format1.monthlyRent_Prior.length()).trim().split(" ")[0];
				    ...
				    if(text.contains(PDFAppConfig.SanAntonio_Format1.HVACFilterAddendumTextAvailabilityCheck))
				    {
				    	...
				    	//HVAC Air Filter Fee
				    	 try
						    {
						    	PDFReader.HVACAirFilterFee = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.HVACAirFilterFee)+PDFAppConfig.SanAntonio_Format1.HVACAirFilterFee.length()).trim().split(" ")[0];
				    ...
				    if(text.contains(PDFAppConfig.SanAntonio_Format1.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
				    {
				    	...
				    	//HVAC Air Filter Fee
				    	 try
						    {
						    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.RBP_Prior)+PDFAppConfig.SanAntonio_Format1.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
				    ...
				    try
				    {
				    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.prorateRent_Prior)+PDFAppConfig.SanAntonio_Format1.prorateRent_Prior.length()).trim().split(" ")[0];
				    ...
				    if(text.contains(PDFAppConfig.SanAntonio_Format1.petAgreementAvailabilityCheck))
				    {
				    	...
				    	try
				    	{
				    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.petRent_Prior)+PDFAppConfig.SanAntonio_Format1.petRent_Prior.length()).trim().split(" ")[0].trim();
				    ...
				    try
			    	{
			    		PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.SanAntonio_Format1.leaseRenewalFee_Prior)+PDFAppConfig.SanAntonio_Format1.leaseRenewalFee_Prior.length()).trim().split(" ")[0].trim();
