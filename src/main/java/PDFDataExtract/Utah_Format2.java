...
public class Utah_Format2 
{
	...
	public static boolean utah() throws Exception
	{
		try
		{
	    ...
	    try {
	        int commencementIndex = text.indexOf(PDFAppConfig.Utah_Format2.commencementDate_Prior) + PDFAppConfig.Utah_Format2.commencementDate_Prior.length();
	        int commencementEndIndex = text.indexOf(PDFAppConfig.Utah_Format2.commencementDate_After, commencementIndex);
	    ...
	    try {
	        int expirationIndex = text.indexOf(PDFAppConfig.Utah_Format2.expirationDate_Prior) + PDFAppConfig.Utah_Format2.expirationDate_Prior.length();
	        int expirationEndIndex = text.indexOf(PDFAppConfig.Utah_Format2.expirationDate_After, expirationIndex);
	    ...
	    try
	    {
	    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.monthlyRent_Prior)+PDFAppConfig.Utah_Format2.monthlyRent_Prior.length()).trim().split(" ")[0];
	    ...
	    if(text.contains(PDFAppConfig.Utah_Format2.HVACFilterAddendumTextAvailabilityCheck))
	    {
	    	PDFReader.HVACFilterFlag = true;
	    	//HVAC Air Filter Fee
	    	 try
			    {
			    	PDFReader.HVACAirFilterFee = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.HVACAirFilterFee)+PDFAppConfig.Utah_Format2.HVACAirFilterFee.length()).trim().split(" ")[0];
	    ...
	    if(text.contains(PDFAppConfig.Utah_Format2.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
	    {
	    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
	    	//HVAC Air Filter Fee
	    	 try
			    {
			    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.RBP_Prior)+PDFAppConfig.Utah_Format2.RBP_Prior.length()).trim().split(" ")[0].replaceAll("[^0-9a-zA-Z.]", "");
    	...
    	try
	    {
    		if(text.indexOf(PDFAppConfig.Boise_Format1.RUBS_Prior)==-1)
    		{
    			...
    			{
    				PDFReader.RUBS = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.RUBS_Prior3)+PDFAppConfig.Utah_Format2.RUBS_Prior3.length()).trim().split(" ")[0];
    			}
    			else {
    			PDFReader.RUBS = text.substring(text.indexOf(PDFAppConfig.Utah_Format1.RUBS_Prior2)+PDFAppConfig.Boise_Format1.RUBS_Prior2.length()).trim().split(" ")[0];
    			}
    		}
    		else {
    			 PDFReader.RUBS = text.substring(text.indexOf(PDFAppConfig.Utah_Format1.RUBS_Prior)+PDFAppConfig.Boise_Format1.RUBS_Prior.length()).trim().split(" ")[0];
    		}
	    }
    	catch(Exception e)
	    {
	    ...
	    try
	    {
	    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.prorateRent_Prior)+PDFAppConfig.Utah_Format2.prorateRent_Prior.length());
	  ...
	  //Lease Renewal Admin Fee
	    try
	    {
	    	PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.leaseRenewalAdminFee_Prior)+PDFAppConfig.Utah_Format2.leaseRenewalAdminFee_Prior.length()).trim().split(" ")[0];
		...
		//Pet Rent
	    if(text.contains(PDFAppConfig.Utah_Format2.petAgreementAvailabilityCheck))
	    {
	    	...
	    	try
	    	{
	    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.petRent_Prior)+PDFAppConfig.Utah_Format2.petRent_Prior.length()).trim().split(" ")[0].trim();
	    ...
	    try
	    {
	    	if(text.contains("on the first 12 months"))
	    	{
	    		...
	    		try
	    		{
	    		PDFReader.increasedRent_amount = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.increasedRent_Prior1)+PDFAppConfig.Utah_Format2.increasedRent_Prior1.length()).trim().split(" ")[0].trim();
	    	...
	    	if(text.contains("and, $"))
	    	{
	    		...
	    		try
	    		{
	    		PDFReader.increasedRent_amount = text.substring(text.indexOf(PDFAppConfig.Utah_Format2.increasedRent_Prior2)+PDFAppConfig.Utah_Format2.increasedRent_Prior2.length()).trim().split(" ")[0].trim();
	    		if(PDFReader.increasedRent_amount.matches(".*[a-zA-Z]+.*")||PDFReader.increasedRent_amount.equals("0.00"))
		    		PDFReader.increasedRent_amount = "Error";
	    		}
	    		catch(Exception e)
