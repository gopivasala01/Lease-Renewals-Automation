...
public class Alabama_Format1 {
	...
	public static boolean alabama() throws Exception
	{
	    ...
	    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
		   ...
		   try
		    {
			   PDFReader.expirationDate = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.expirationDate_Prior)+PDFAppConfig.Alabama_Format1.expirationDate_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.expirationDate_After));
		  ...
		  /*  try
		   ...
		   if(text.contains(PDFAppConfig.Alabama_Format1.HVACFilterAddendumTextAvailabilityCheck))
		    ...
		    if(text.contains(PDFAppConfig.Alabama_Format1.residentBenefitsPackageCheck)&&(!text.contains("Resident Benefits Package Opt-Out Addendum")||!text.contains("RESIDENT BENEFITS PACKAGE OPT-OUT ADDENDUM")))
		    {
		    	PDFReader.residentBenefitsPackageAvailabilityCheck = true;
		    	//HVAC Air Filter Fee
		    	 try
				    {
				    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.RBP_Prior)+PDFAppConfig.Alabama_Format1.RBP_Prior.length()).trim().split(" ")[0];
		    ...
		    try
		    {
		    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.prorateRent_Prior)+PDFAppConfig.Alabama_Format1.prorateRent_Prior.length()).trim().split(" ")[0];
			...
			//Pet Rent
		    if(text.contains(PDFAppConfig.Alabama_Format1.petAgreementAvailabilityCheck))
		    {
		    	...
		    	try
		    	{
		    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.petRent_Prior)+PDFAppConfig.Alabama_Format1.petRent_Prior.length()).trim().split(" ")[0].trim();
		    ...
		    try
	    	{
	    		PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.leaseRenewalFee_Prior)+PDFAppConfig.Alabama_Format1.leaseRenewalFee_Prior.length()).trim().split(" ")[0].trim();
