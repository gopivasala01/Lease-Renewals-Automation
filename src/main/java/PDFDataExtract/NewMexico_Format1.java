				   ...
				   System.out.println("Expiration Date = "+PDFReader.expirationDate);
				    ...
				    if(text.contains(PDFAppConfig.NewMexico_Format1.residentBenefitsPackageCheck)&&!text.contains("Resident Benefits Package Opt-Out Addendum"))
				    {
				    	...
				    	//HVAC Air Filter Fee
				    	 try
						    {
						    	PDFReader.residentBenefitsPackage = text.substring(text.indexOf(PDFAppConfig.NewMexico_Format1.RBP_Prior)+PDFAppConfig.NewMexico_Format1.RBP_Prior.length()).trim().split(" ")[0];
