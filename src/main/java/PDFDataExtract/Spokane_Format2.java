		...
		public static boolean spokane() throws Exception
		{
		    ...
		    System.out.println("Last date mentioned on the page: " + PDFReader.renewalExecutionDate);
			   ...
			   System.out.println("Expiration Date = "+PDFReader.expirationDate);
			    ...
			    try
			    {
			    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.Spokane_Format1.monthlyRent_Prior)+PDFAppConfig.Spokane_Format1.monthlyRent_Prior.length()).trim().split(" ")[0];
			    ...
			    if(text.contains(PDFAppConfig.Spokane_Format1.HVACFilterAddendumTextAvailabilityCheck))
			    {
			    	...
			    	//HVAC Air Filter Fee
			    	 try
					    {
					    	PDFReader.HVACAirFilterFee = text.substring(text.indexOf(PDFAppConfig.Spokane_Format1.HVACAirFilterFee)+PDFAppConfig.Spokane_Format1.HVACAirFilterFee.length()).trim().split(" ")[0];
			    ...
			    try
			    {
			    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Spokane_Format1.prorateRent_Prior)+PDFAppConfig.Spokane_Format1.prorateRent_Prior.length()).trim().split(" ")[0];
			    ...
			    if(text.contains(PDFAppConfig.Spokane_Format1.petAgreementAvailabilityCheck))
			    {
			    	...
			    	try
			    	{
			    		PDFReader.petRent = text.substring(text.indexOf(PDFAppConfig.Spokane_Format1.petRent_Prior)+PDFAppConfig.Spokane_Format1.petRent_Prior.length()).trim().split(" ")[0].trim();
			    ...
			    try
		    	{
		    		PDFReader.leaseRenewalFee = text.substring(text.indexOf(PDFAppConfig.Spokane_Format1.leaseRenewalFee_Prior)+PDFAppConfig.Spokane_Format1.leaseRenewalFee_Prior.length()).trim().split(" ")[0].trim();
